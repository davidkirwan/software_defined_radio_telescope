...
Jupiter is visible for a number of months each year and generally in the evening, night, or morning hours. As the Earth's ionosphere begins to heats up during the day, it becomes opaque to signals in the \gls{DAM} band which can prevent weaker signals being captured by a ground based listening site (get citation for this). 

Interference from human sources such as short wave radio stations or amateur radio transmissions is likely to affect the collection of \gls{DAM} emissions from Jupiter. Want to develop a system for the automatic flagging of human transmissions using either signal processing or making use of manually identified transmissions using a dx cluster

Solar storm activity can be captured during daylight hours, providing the antenna deployment configuration covers the part of the sky that the Sun traverses during the daylight hours. 

\subsection*{Problem Characteristics}
\addcontentsline{toc}{subsection}{Problem Characteristics}

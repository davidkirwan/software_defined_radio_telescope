...
\section*{Capturing DAM Spectrograms with rtl\_power and heatmap.py and the RTL2832 DAB SDR}
\addcontentsline{toc}{section}{Capturing DAM Spectrograms with rtl\_power and heatmap.py and the RTL2832 DAB SDR}

Due to its recent release there is a distinct lack of tool support available which is capable of interfacing with the HackRF and generating a spectrogram plot from saved data. A system is available for the RTL DAB Digital TV Tuner however. The \textit{rtl\_power} utility is shipped with the firmware required to operate the \gls{SDR} receiver and is capable of this functionality. The RTL systems have a similar operating range to the HackRF and align somewhat closely with the requirements of the SDRT system in order to pick up \gls{DAM} emissions from Jupiter \citep{hamradioscience-12}. Table: \ref{tab:rtl_vs_hackrf} lists the operating ranges of both the HackRF and also the RTL DAB receiver.
...
The RTL DAB receiver operates at 24MHz at the lower end of its operational range. This is still well within the 18MHz - 40MHz range of Jovian \gls{DAM} emissions, but it is just outside the ideal range recommended by NASA's Radio Jove documentation \cite{nasa12}.
...
\begin{enumerate}
	...
	\item rtl\_power was left running for 24 hours
	\item Spectrogram was generated using the heatmap.py utility
\end{enumerate}

...
\citet{nelson-15} implemented a total power radiometer within software as part of a M.Sc thesis. The equation listed in Figure: \ref{fig:power_formula} has been implemented in a GNURadio flow-graph. The functionality of a radiometer or total power meter provides the means to measure a chunk of bandwidth from a sample and produce a total noise power value which can then be plotted or used elsewhere within a larger GNURadio flow-graph.
...
In order to use the radiometer for radio astronomical applications it would first need to be calibrated against known sources to convert power readings to signal intensity measured in Kelvins \citep{nrao-10}.

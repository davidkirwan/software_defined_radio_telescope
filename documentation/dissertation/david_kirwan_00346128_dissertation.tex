...
A software client was developed in order to connect to a local server within the DX Cluster. It was designed to gather all data from the cluster while filtering transmissions outside the frequency range currently being monitored by the telescope. All signals which have been identified as being within the range being monitored, would be flagged and all related meta data for each transmission would be stored for processing at a later stage.
...
\begin{enumerate}
	\item Collect 24 hours worth of data
	\item Filter all signals not originating in Europe
	\item Filter all signals outside the target range
\end{enumerate}
